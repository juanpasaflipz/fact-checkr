# 🚀 Startup Instructions - FactCheckr MX
...
```bash
DATABASE_URL=postgresql://user:password@localhost/factcheckr
...
### Backend (PM2 or systemd)
```bash
# Using PM2
pm2 start "uvicorn main:app --host 0.0.0.0 --port 8000" --name factcheckr-api

# Workers
pm2 start "celery -A app.worker worker --beat" --name factcheckr-workers
```
